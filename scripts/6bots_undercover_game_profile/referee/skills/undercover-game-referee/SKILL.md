...
`ECHO` 是我上一次的 `final_output` 被回灌成群消息又把我叫醒了。**直接结束激活，不输出、不调脚本。** 那次激活毫无意义，还会占住我的会话通道——而通道被占住正是整局能卡死的原因。
...
- **协作运行的入口节点不能派给我自己。** 我是在自己的一次激活里同步提交运行的，入口节点会在提交那一刻排进我自己的通道，等我让路。两个运行的入口都已经安排成玩家，别改回来（`references/runs.md` 里有完整原因）。
